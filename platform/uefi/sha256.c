/* SPDX-License-Identifier: MIT */
/* Portable SHA-256; no SIMD/crypto ISA assumption at EFI or EL2. */
#include "sha256.h"
static uint32_t ror(uint32_t x, unsigned n) { return (x >> n) | (x << (32-n)); }
static void transform(struct q1n1_sha256 *s, const uint8_t *p)
{
    static const uint32_t k[64] = {
        0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
        0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
        0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
        0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
        0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
        0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
        0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
        0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
    };
    uint32_t w[64];
    for (unsigned i=0;i<16;i++) w[i]=(uint32_t)p[i*4]<<24|(uint32_t)p[i*4+1]<<16|(uint32_t)p[i*4+2]<<8|p[i*4+3];
    for (unsigned i=16;i<64;i++) {
        uint32_t a=w[i-15],b=w[i-2];
        w[i]=w[i-16]+(ror(a,7)^ror(a,18)^(a>>3))+w[i-7]+(ror(b,17)^ror(b,19)^(b>>10));
    }
    uint32_t a=s->h[0],b=s->h[1],c=s->h[2],d=s->h[3],e=s->h[4],f=s->h[5],g=s->h[6],h=s->h[7];
    for (unsigned i=0;i<64;i++) {
        uint32_t t=h+(ror(e,6)^ror(e,11)^ror(e,25))+((e&f)^(~e&g))+k[i]+w[i];
        uint32_t u=(ror(a,2)^ror(a,13)^ror(a,22))+((a&b)^(a&c)^(b&c));
        h=g;g=f;f=e;e=d+t;d=c;c=b;b=a;a=t+u;
    }
    s->h[0]+=a;s->h[1]+=b;s->h[2]+=c;s->h[3]+=d;s->h[4]+=e;s->h[5]+=f;s->h[6]+=g;s->h[7]+=h;
}
void q1n1_sha256_init(struct q1n1_sha256 *s)
{
    static const uint32_t h[8]={0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19};
    for(unsigned i=0;i<8;i++) s->h[i]=h[i];
    s->bytes=0;
}
void q1n1_sha256_update(struct q1n1_sha256 *s, const void *data, size_t size)
{
    const uint8_t *p=data;
    size_t used=s->bytes%64;
    s->bytes+=size;
    if(used) {
        while(size && used<64) {s->block[used++]=*p++;size--;}
        if(used==64) transform(s,s->block);
        else return;
    }
    while(size>=64) {transform(s,p);p+=64;size-=64;}
    for(size_t i=0;i<size;i++) s->block[i]=p[i];
}
void q1n1_sha256_final(struct q1n1_sha256 *s, uint8_t digest[32])
{
    uint64_t bits=s->bytes*8;
    size_t used=s->bytes%64;
    s->block[used++]=0x80;
    if(used>56) {while(used<64) s->block[used++]=0;transform(s,s->block);used=0;}
    while(used<56) s->block[used++]=0;
    for(unsigned i=0;i<8;i++) s->block[63-i]=(uint8_t)(bits>>(8*i));
    transform(s,s->block);
    for(unsigned i=0;i<32;i++) digest[i]=(uint8_t)(s->h[i/4]>>(24-8*(i%4)));
}
void q1n1_sha256(const void *data, size_t size, uint8_t digest[32])
{
    struct q1n1_sha256 s;
    q1n1_sha256_init(&s);q1n1_sha256_update(&s,data,size);q1n1_sha256_final(&s,digest);
}
